$(document).ready(function() {
    ...
    var d = "http://" + window.location.host + "/rest/list-organisations";
    var l = "http://" + window.location.host + "/rest/list-mapping-dir-contents";
    var c = "http://" + window.location.host + "/rest/list-output-dir-contents";
    var p = "http://" + window.location.host + "/rest/list-input-dir-contents";
    var e = "http://" + window.location.host + "/rest/list-xquery-dir-contents";
    if (toolType === 'both') {
        ...
        $('.tool_navigation a').on('click', function(event) {
            ...
            if (activeTool === 'validation') {
                $('#submit_step1').prop("disabled", false)
                $('#step6').hide();
                $('#step1').hide();
                $('#previous_step5').hide();
                $('#step5').show();
                ...
                $('#submit_step5').html('Start Validation')
                $('#submit_step5').attr('id', 'startValidation');
                s();
                $('#income_folder').empty();
                $('#errors_table').empty();
                $('#startValidation').on('click', function() {
                    $('#income_folder').empty();
                    $('#errors_table').empty();
                    B = "http://" + window.location.host + "/rest/validate";
                    $.get(B, function(G) {
                        var I = G;
                        var H = 0;
                        ...
                        $('.getContent').on('click', function() {
                            ...
                            var settings = { "async": true, "crossDomain": true, "url": 'http://' + window.location.host + '/rest//htmlReport?path=' + addressForCall, "method": "GET", "headers": { "cache-control": "no-cache" } }
                            $.ajax(settings).done(function(response) {
                                var fileName = settings.url
                                var result = fileName.substring(fileName.lastIndexOf("\\") + 1);
                                result = result.replace(".html", "");
                                var name = result.substring(result.lastIndexOf("/") + 0);
                                name = name.replace("/", "");
                                $('#step6').append('<div class="responseHolder"><h2 style="margin-top: -33px;">' + name + '</h2><p>File location: '+result+'</p><span class="closeResponse">X</span>' + response + '</div>');
                        ...
                        $('.success_notes p').html('File was Validated Successfully!');
                        $('#start_new').html('Start new Validation');
                        $('#start_new').attr('id', 'start_new_validation');
                        $('#repeat').html('Repeat Validation');
                        $('#start_new_validation').on('click', function(){
                            $("#step5").slideDown(300);
                            $("#step6").slideUp(300);
                            $(".active").removeClass("active");
                            $("#label_step_5").addClass("active");
                ...
                $('#submit_step5').html('Start Transformation')
                $('#startValidation').attr('id', 'submit_step5');
                $('.success_notes p').html('Data was transformed Successfully Successfully!');
                $('#start_new').html('Start new transformation');
                $('#repeat').html('Repeat transformation');
            }
        });
        ...
        s();
        $('#startValidation').on('click', function() {
            console.log('validation call now')
            B = "http://" + window.location.host + "/rest/validate";
            $.get(B, function(G) {
                var I = G;
                var H = 0;
    ...
    function w() {
        ...
        if ($("#file_type").val() === "xml_ead") {
            B = "http://" + window.location.host + "/rest/process?fileType=xml_ead";
            $.get(B, function(G) {
                var I = G;
                var H = 0;
                $.each(I.split("|"), function(K, L) {
            ...
            if ($("#mapping_type").val() === "generic" && C != "") {
                var fileTypeParam = $("#file_type").val();
                B = "http://" + window.location.host + "/rest/process?fileType="+fileTypeParam+"?mapping=" + C;
                $.get(B, function(G) {
                    var I = G;
                    var H = 0;
                    $.each(I.split("|"), function(K, L) {
                ...
                if ($("#mapping_type").val() === "generic" && C === "") {
                    var fileTypeParam = $("#file_type").val();
                    B = "http://" + window.location.host + "/rest/process?fileType="+fileTypeParam+"&organisation=" + F;
                    $("#errors_table").empty();
                    $.get(B, function(G) {
                        var I = G;
                        var H = 0;
                    ...
                    if ($("#mapping_type").val() === "specific") {
                        var fileTypeParam = $("#file_type").val();
                        B = "http://" + window.location.host + "/rest/process?fileType="+fileTypeParam+"?xquery=" + E;
                        $.get(B, function(G) {
                            var I = G;
                            var H = 0;
                            $.each(I.split("|"), function(K, L) {
                        ...
                        if ($("#mapping_type").val() === "generic") {
                            if (navigator.onLine) {
                                if ($("#organization").val() != "no_organization") {
                                    var D = "http://" + window.location.host + "/rest/mapping-sheet-ID?organisation=" + $("#organization").val();
                                    $.get(D, function(F) {
